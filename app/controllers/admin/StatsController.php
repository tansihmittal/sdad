...
class Stats {	
    ...
    public function subscriptions(){
       ...
       $results = Helper::cacheGet('chartsubscriptions');

        if($results === null){

            $results = DB::subscription()->selectExpr('COUNT(MONTH(date))', 'count')->selectExpr('DATE_FORMAT(date, "%Y-%m")', 'newdate')->whereRaw("(DATE(date) >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH))")->groupByExpr('newdate')->findArray();
            Helper::cacheSet('chartsubscriptions', $results,  60 * 60);
        }
    ...
    public function payments(){
       ...
       $results = Helper::cacheGet('chartpayment');
        ...
        if($results === null){
            ...
            Helper::cacheSet('chartpayment', $results,  60 * 60);
        }
