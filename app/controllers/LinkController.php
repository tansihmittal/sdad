...
class Link {
	...
	*/
    ...
    public function qr(Request $request, string $alias, int $size = 300, $action = "view"){		
		...
		return \Helpers\QR::factory($url, $qrsize)->color('rbg(255,255,255)', 'rbg(0,0,0)')->format('svg')->create();
    }
	/**
	 * Download QR
	 *
	...
	public function qrDownload(Request $request, string $alias, string $format, int $size = 300){
		...
		$qr = \Helpers\QR::factory($url, $qrsize)->format($format)->color('rbg(255,255,255)', 'rbg(0,0,0)');
