<div class="container pt-5 pb-2" id="profile">
                    ...
                    <?php if(isset($profiledata['social'])): ?>
                        <div id="social" class="text-center mt-3">
                            <?php foreach($profiledata['social'] as $key => $value): ?>
                                ...
                                <a href="<?php echo $value ?>" class="ms-3" target="_blank" data-bs-toggle="tooltip" data-placement="top" title="<?php echo ucfirst($key) ?>" rel="nofollow"><i class="fab fa-<?php echo $key ?>"></i></a>
                            <?php endforeach ?>
                        </div>
                    <?php endif ?>
                <?php endif ?>
                ...
                <?php foreach($profiledata['links'] as $id => $value): ?>
                    <div class="item mb-3">
                        ...
                        <?php if($value['type'] == "rss"): ?>
                            <div class="rss card card-body overflow-auto btn-custom rounded">
                                <?php $items = \Helpers\App::rss($value['link']) ?>
                                <?php if(!is_array($items)): ?>
                                    <?php echo $items ?>
                                <?php else: ?>
                                    <?php foreach($items as $item): ?>
                                        <div class="media mb-3 text-start">
                                            <?php if($item['image']): ?>
                                                <img class="me-3" src="<?php echo $item['image'] ?>" alt="<?php echo $item['title'] ?>">
                                            <?php endif ?>
                        ...
                        <?php if($value['type'] == "whatsapp"): ?>
                            <a href="https://wa.me/<?php echo str_replace([' ', '-'], '', $value['phone']) ?>" class="btn btn-block d-block p-3 btn-custom position-relative"><img src="<?php echo assets('images/whatsapp.svg') ?>" height="26" class="ms-3 position-absolute start-0"> <?php echo isset($value['label']) && $value['label'] ? $value['label'] : $value['phone'] ?></a>
                        <?php endif ?>

                        <?php if($value['type'] == "whatsappmessage"): ?>
                            <a href="https://wa.me/<?php echo str_replace([' ', '-'], '', $value['phone']) ?>?text=<?php echo urlencode(clean($value['message'], 3)) ?>" class="btn btn-block d-block p-3 btn-custom position-relative"><img src="<?php echo assets('images/whatsapp.svg') ?>" height="26" class="ms-3 position-absolute start-0"> <?php echo isset($value['label']) && $value['label'] ? $value['label'] : $value['phone'] ?></a>
                        <?php endif ?>

                        <?php if($value['type'] == "phone"): ?>
                            <a href="tel:<?php echo str_replace([' ', '-'], '', $value['phone']) ?>" class="btn btn-block d-block p-3 btn-custom position-relative"><i class="fa fa-phone ms-3 position-absolute start-0"></i> <?php echo isset($value['label']) && $value['label'] ? $value['label'] : $value['phone'] ?></a>
                        <?php endif ?>
                        ...
                        <?php if($value['type'] == "tiktok"): ?>
                            <blockquote class="tiktok-embed rounded btn-custom" cite="<?php echo $value['link'] ?>" data-video-id="<?php echo $value['id'] ?>" style="max-width: 660px;min-width: 325px;" > <section> </section> </blockquote> <script async src="https://www.tiktok.com/embed.js"></script>
                        <?php endif ?>
                        <?php if($value['type'] == "html"): ?>
                            <?php echo $value['html'] ?>
                        <?php endif ?>
                        <?php if($value['type'] == "newsletter"): ?>
                            <a href="#" data-bs-target="#N<?php echo $id ?>" data-bs-toggle="collapse" role="button" class="btn btn-block p-3 d-block btn-custom position-relative">
                        ...
                        <?php if($value['type'] == "contact"): ?>
                            <a href="#" data-bs-target="#C<?php echo $id ?>" data-bs-toggle="collapse" role="button" class="btn btn-block p-3 d-block btn-custom position-relative">
                                <span class="align-top"><?php echo $value['text'] ?></span>
                                <i class="fa fa-chevron-down position-absolute end-0 me-3"></i>
                            </a>
                            <form method="post" action="#" id="C<?php echo $id ?>" class="collapse border rounded-3 text-start p-3 mt-3">
                        ...
                        <?php if($value['type'] == "product"): ?>
                            <a href="<?php echo $value['link'] ?>" target="_blank" class="d-block btn-custom rounded-3 p-2 mt-2 text-start text-left" rel="nofollow">
                                <div class="d-flex align-items-center">
    ...
    <div class="text-center mt-3 opacity-8">
        <a class="navbar-brand me-0" href="<?php echo route('home') ?>">
            <?php if(config('logo')): ?>
                <img alt="<?php echo $sitetitle ?? ''?>" src="<?php echo uploads(config('logo')) ?>" id="navbar-logo">
            <?php else: ?>
