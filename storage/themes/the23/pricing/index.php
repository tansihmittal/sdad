            ...
            <div class="pricing-container">
                <div class="text-center mb-5">
                    <div class="bg-white d-inline-block shadow-sm mb-5 p-2 rounded-pill" role="group" aria-label="Pricing">
                        ...
                        <button type="button" class="px-4 px-md-5 btn <?php echo $default == 'lifetime' ? 'btn-primary' : 'btn-white' ?> fw-bold rounded-pill" data-pricing="lifetime"><?php ee('Lifetime') ?></button>
                        <?php endif ?>
                        <?php if($settings['monthly']):?>
                        <button type="button" class="px-4 px-md-5 btn <?php echo $default == 'monthly' ? 'btn-primary' : 'btn-white' ?> fw-bold rounded-pill" data-pricing="monthly"><?php ee('Monthly') ?></button>
                        <?php endif ?>
                        <?php if($settings['yearly']):?>
                        <button type="button" class="px-4 px-md-5 btn <?php echo $default == 'yearly' ? 'btn-primary' : 'btn-white' ?> fw-bold rounded-pill position-relative" data-pricing="yearly">
                            <span><?php ee('Yearly') ?></span>
                            <?php if($settings['discount']): ?>
                                <span class="badge bg-danger border-0 badge-pill position-absolute start-100 ms-0 ms-md-2 top-0 translate-middle shadow-sm"><?php ee('Save') ?> <?php echo $settings['discount'] ?>%</span>
                            <?php endif ?>
                        </button>
                        <?php endif ?>
                    </div>
